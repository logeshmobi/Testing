...
public class RecurringTxnListExcel extends AbstractExcelView {
	...
	public void setExcelHeader(HSSFSheet excelSheet) {
		...
		excelHeader.createCell(4).setCellValue("No Of Payments");
		excelHeader.createCell(5).setCellValue("Frequency");
	...
	public void setExcelRows(HSSFSheet excelSheet, List<EzyRecurringPayment> txnList){
		...
		for (EzyRecurringPayment txn : txnList) {
			...
			excelRow.createCell(4).setCellValue(txn.getInstallmentCount());
			excelRow.createCell(5).setCellValue(txn.getPeriod());
