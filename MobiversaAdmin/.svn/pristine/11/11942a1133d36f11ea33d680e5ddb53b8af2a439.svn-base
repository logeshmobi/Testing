...
<%@taglib prefix="sec" uri="http://www.springframework.org/security/tags"%>
	...
	<div>
	<div>Search for a Reader</div>
	<div>
		<form method="get" action="<%=ReaderController.URL_BASE%>/search">
...
<div class="portlet-body flip-scroll">
	...
	<c:choose>
			<c:when test="${reader.username == null}">
	<table
		...
		<thead class="flip-content">
			...
			<tr>
				...
				<th scope="col">MID/(BusinessName)</th>
		...
		<tbody>
			<c:forEach items="${paginationBean.itemList}" var="dto" >
				<tr>
					...
					<td>${dto.merchant.mid.mid} --- ${dto.merchant.businessName}<br><a href="/reader/viewReaderList/${dto.merchant.id}">view all readers</a></td>
					<td><a class="btn btn-primary" href="/reader/details/${dto.id}">View</a></td>
				</tr>
			</c:forEach>
	  </tbody>
	</table>
			...
			<c:otherwise>
				...
				<p style="color:red;"> Your are Currently viewing user: ${reader.username }</p>
			</c:otherwise>
</c:choose>
