<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
	...
	<c:choose>
		<c:when test="${paginationBean.previousPageAvailable }">
			<li><a href="#" onclick="loadData(${paginationBean.currPage -1})">&laquo;</a></li>
		</c:when>
		<c:otherwise>
	...
	<c:forEach items="${paginationBean.paginationNumbers }" var="num">
		<c:choose>
			...
			<c:otherwise>
				<li><a href="#" onclick="loadData(${num})">${num}</a></li>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:choose>
		<c:when test="${paginationBean.nextPageAvailable }">
			<li><a href="#" onclick="loadData(${paginationBean.currPage +1})">&raquo;</a></li>
		</c:when>
		<c:otherwise>
