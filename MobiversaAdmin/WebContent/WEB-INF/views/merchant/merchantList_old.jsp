	...
	function loaddata() {
		...
		if (e == null || e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/admmerchant/export?date='
					+ fromdateString + '&date1=' + todateString + '&export=' + e2 + '&type=' + e3;
			//alert(e);
			form.submit();
            ...
            <table id="data_list_table" class="table table-striped table-bordered">
              ...
              <c:forEach items="${paginationBean.itemList}" var="merc">
                <tr>
                 ...
                 <td>${merc.mid.motoMid}</td>
                 </c:when>
                 <c:otherwise>
                 <td></td>
                 </c:otherwise>
                 </c:choose>
                 <td>${merc.mid.ezypassMid}</td>
                 <td>${merc.mid.ezyrecMid}</td>
                 <td>${merc.mid.ezywayMid}</td>
                   <c:choose>
                 ...
                 <td>${merc.mid.splitMid}</td>  
                 <td>${merc.mid.umMid}</td>               
                 <td>${merc.mid.umEzywayMid}</td>
                  <c:choose>
	         		<c:when test = "${merc.auth3DS == null || merc.auth3DS == '' || merc.auth3DS == 'No' && merc.merchantType == 'U'}">
                  <td>${merc.mid.umMotoMid}</td>
                  </c:when>
                  <c:otherwise>
                 <td></td>
                 </c:otherwise>
                  ...
                  <c:choose>
	         		<c:when test = "${merc.auth3DS == 'Yes' &&  merc.merchantType == 'U'}">
                  <td>${merc.mid.umMotoMid}</td>
                  </c:when>
                  <c:otherwise>
                 <td></td>
                 ...
                 <td>${merc.mid.umEzyrecMid}</td>
                  <!-- <td><i class="material-icons">edit</i></td> -->
                  <td align="center"><a href="${pageContext.request.contextPath}/admmerchant/detail/${merc.id}">
