...
<script lang="JavaScript">
	...
	/* 	function loadExpData() {
	 ...
	 var fromdateString = (frommon <= 9 ? '0' + frommon : frommon) + '/' + fromyear;
	 ...
	 document.location.href = '${pageContext.request.contextPath}/transaction/exportFpxMonthlySettlementSummary?fromDate='
	 ...
	 */
	 ...
	 function loadExpData() {
		    ...
		    // Retrieve the selected username for the chosen business name
		
	
		    var fromDateObj = new Date(fromDate);
		    var fromday = fromDateObj.getDate();
		    var frommon = fromDateObj.getMonth() + 1;
		    var fromyear = fromDateObj.getFullYear();

		    var fromdateString = (frommon <= 9 ? '0' + frommon : frommon) + '/' + fromyear;

		    if (!fromDate || !businessName || !exportType || exportType !== 'EXCEL') {
		        alert("Please Select Date, choose a Business Name, and select Excel as export type");
		    } else {
		        document.getElementById("datevalex1").value = fromdateString;
		        
		        // Check if the element exists before setting its value
		        var selectedUsernameElement = document.getElementById("selectedUsername");
		        if (selectedUsernameElement) {
		            selectedUsernameElement.value = selectedUsername;  // Set the value only if the element exists
		        } else {
		            console.error("selectedUsername element not found");
		        }

		        // Modify the URL to include the selected username
		        var url = '${pageContext.request.contextPath}/transaction/exportBoostMonthlySettlementSummary?' +
		                  'fromDate=' + fromdateString + '&toDate=' + fromdateString + '&id=' + "" + '&export=' + exportType +
				...
				<div class="card blue-bg text-white">
					<div class="card-content">
						<div class="d-flex align-items-center">
							<h3 class="text-white">
								<strong> Boost Monthly Settlement File</strong>
							</h3>
						</div>
					</div>
				</div>
					...
					<div class="card-content padding-card">
						...
						<div class="row">
							<div class="input-field col s12 m3 l3">
								<label for="from" style="margin: 0px;">Select Date </label> <input
							...
							<div class="input-field col s12 m3 l3">
								<select name="businessName" class="Selectoption" id="businessName"
									class="form-control" style="width: 100%">
									<option selected value="">Choose Business Name</option>
									<c:forEach var="entry" items="${businessNamesAndUsernames}">
										<c:if test="${entry[0]!=''}">
										<option value="${entry[1]}">${entry[0]}</option>
										</c:if>
									</c:forEach>
								</select> <label for="businessName">Business Name</label>
							</div>
							...
							<div class="input-field col s12 m3 l3">
								<select name="export" id="export" class="form-control"
									style="width: 100%" onchange="loadDropDate13()">
							...
							<div class="input-field col s12 m3 l3"
								style="float: left; padding-right: 120px">
								<div class="button-class">
									<input type="hidden" name="date1" id="dateval1"> <input
										type="hidden" name="date2" id="dateval2"> <input
										type="hidden" name="dateex1" id="datevalex1"> <input
										type="hidden" name="dateex2" id="datevalex2"> <a
