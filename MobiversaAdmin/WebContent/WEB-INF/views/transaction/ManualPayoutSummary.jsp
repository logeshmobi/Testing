...
String eyeimg = resource.getString("EYESEEIMAGE");
String updateimg = resource.getString("UPDATEIMAGE");
%>
...
<script lang="JavaScript">
	function loadSelectData() {
		$("#overlay").show();
		/* var mid = document.getElementById("umezywaymid").value; */
		//alert("mid:::::"+mid);
		...
		if (e == null || e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/transaction/searchUMEzyway?date='
					+ fromdateString + '&date1=' + todateString+ '&mid=' + mid + '&currPage=' + PageNumber;
			form.submit;
	...
	function loadDropSearch(){
	  	  var e = document.getElementById("drop_search");
	      	...
	      	if (strUser == "Payout_ID") {
	      		document.getElementsByName('search')[0].placeholder = 'Ex: Payout ID';
	      	  } else if (strUser == "Transaction_ID") {
	      		  document.getElementsByName('search')[0].placeholder = 'Ex: Transaction ID';
	      	  }else if (strUser == "Assignee_Name") {
	      		  document.getElementsByName('search')[0].placeholder = 'Ex: Assignee Name';
	      	  }else{
	 ...
	 function loadSearch()
     {
   		...
   		document.location.href = '${pageContext.request.contextPath}/adminsearchNew/findCard?VALUE='
	...
	function loadSelectData1() {
		$("#overlay").show();
		var mid = document.getElementById("TXNType1").value;
		//alert("mid:::::"+mid);
		//alert("test"+document.getElementById("txnType").value);
	...
	function loadDropDate14() {
		...
		var strUser = e.options[e.selectedIndex].value;
		document.getElementById("umezywaymid").value = strUser;
	...
	function loadExpData() {
		...
		if (e == null || e == '' || e1 == null || e1 == '') {
			...
			/* document.location.href = '${pageContext.request.contextPath}/transaction/searchUMEzyway?date='
					+ e + '&date1=' + e1 + '&txnType=' + e2; */
			document.location.href = '${pageContext.request.contextPath}/transaction/umEzywayExport?fromDate=' + fromdateString
					+ '&toDate=' + todateString +'&export='+e2+ '&mid=' + mid; 
			form.submit;
			//document.getElementById("dateval1").value = e;
			//document.getElementById("dateval2").value = e1;
					...
					<div class="card-content padding-card">
						<div class="row">
							...
							<div class="input-field col s12 m3 l3">
								...
								</select> <label class="name">Export Type</label>
							</div>
<!-- 
						</div>
...
/* status confirm style  */

.statusconfirmcontent{
	border:none !important;
	display: flex;
	flex-direction: column;
	align-items: center;
...
.statusconfirmpopup .modal-content {
    padding: 0px 28px;
}
...
input[type="radio"]:not(:checked) + span:before, [type="radio"]:not(:checked) + span:after {
...
#declinedreason::placeholder {
        font-size: 12px;
        color: #929292; 
    }

input[type="radio"] + span:before, [type="radio"] + span:after {
    content: '';
    position: absolute;
    left: 0;
    top: 2px;
							...
							<table id="data_list_table1"
								...
								<tbody>
									...
									<tr>
											<td>2024-02-13</td>
											<td>12:14 PM</td>
											<td>EXIM TECHNOLOGIES</td>
											
											<td>Hanfa hilma bin Abdhul halim</td>
											<td>5454545430004343</td>
											<td id="payoutxnstatus" class="statuscoulumn" ><img src = "${pageContext.request.contextPath}/resourcesNew1/assets/Declined.svg"  width="20" height="20"/><span>Failure</span> </td>
											<td>100.00</td>
											<td>Maybank Berhad</td>
											<td>100043455056</td>
											<td>3420434535356456</td>
										
											<td>Sathish kumar</td>
												<td class='center'>
												 <button class="eyebtn modal-trigger" type="button" data-target="declineReasonModal" onclick="declineReason()"><img src = "${pageContext.request.contextPath}/resourcesNew1/assets/seereason.svg"
													class="w24" <%-- src='data:image/png;base64,<%=eyeimg%>  --%>' /></button>
												</td>
            								
												<td class='center'>
												<button class="updatebtn modal-trigger" type="button" data-target="statusConfirmModal" onclick="statusConfirm()">
												<img src = "${pageContext.request.contextPath}/resourcesNew1/assets/Update.svg"
													class="w24" <%-- src='data:image/png;base64,<%=updateimg%> --%> ' /></button>
												</td>
...
<c:forEach var="payout" items="${payouts}">
    <tr>
        ...
        <td>${payout.accountNo}</td>

        <td style="display: flex; align-content: center;justify-content: center;align-items: center;margin-top: 3px;">
            <c:choose>
                <c:when test="${payout.status eq 'Success'}">
                   <img src="${pageContext.request.contextPath}/resourcesNew1/assets/success.svg" width="20" height="20"/><span style="color:green;"> ${payout.status}</span> 
                </c:when>
                <c:when test="${payout.status eq 'Failure'}">
                   <img src="${pageContext.request.contextPath}/resourcesNew1/assets/Declined.svg"  width="20" height="20" /><span style="color:red;"> ${payout.status}</span> 
                </c:when>
                <c:when test="${payout.status eq 'Bank-Declined'}">
                  <img src="${pageContext.request.contextPath}/resourcesNew1/assets/Declined.svg"  width="20" height="20" /> <span style="color:#red;"> ${payout.status}</span> 
                </c:when>
                <c:otherwise>
                   <img src="${pageContext.request.contextPath}/resourcesNew1/assets/Process.svg"  width="20" height="20" /> <span style="color:#ddd;"> ${payout.status}</span>
                </c:otherwise>
            </c:choose>
        </td>
        ...
        <td>${payout.assigneeName}</td>
        
        <td class='center'>
            <button class="eyebtn" type="button">
                <img src="${pageContext.request.contextPath}/resourcesNew1/assets/seereason.svg"/>
            </button>
        </td>
	 ...
	 <div id="declineReasonModal" class="modal declinereasonpopup">
  	
  	<div class="declineReasonModal_heading" style="border-bottom: 2px solid #ffa500; padding:1rem 1rem; font-size: 16px; color:#005baa;text-align:center; font-weight: 600; font-family: 'Poppins', sans-serif;">Decline Reason</div>
  
   <form action="${pageContext.request.contextPath}/transactionUmweb/payoutbalance" id="declineReasonForm" method="get"  >
		    <div class="modal-content declinereasoncontent ">    
		    	
		    	<p style="font-size: 15px; text-align: center">Host is Busy Now!</p>
		 		   	
		    </div>
    
		    <div class="modal-footer statusbtns" style="padding: 0% 5%; display: flex; flex:1; flex-direction: row; align-items: center; justify-content: space-evenly; background-color: #fff; border: none;">
				<button type="button"  id="cancelBtn" class="btn btn-secondary modal-close " id="" style="width: 30%; background-color: #fff; color: #005baa; border: 1px solid #005baa; height: 35px; font-size: 12px; border-radius: 50px !important; font-family: 'Poppins', sans-serif; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Cancel</button>
			</div>
	</form>
  </div>
	...
	<!-- status confirm -->
	 ...
	 <div id="statusConfirmModal" class="modal statusconfirmpopup">
  	
  	<div class="statusConfirmModal_heading" style="border-bottom: 2px solid #ffa500; padding:1rem 1rem; font-size: 16px; color:#005baa;text-align:center; font-weight: 600; font-family: 'Poppins', sans-serif;">Status Confirmation</div>
  
   <form action="${pageContext.request.contextPath}/transactionUmweb/payoutbalance" id="statusConfirmForm" method="get"  >
		    <div class="modal-content ">    
		    	
		    	<div class="statusconfirmcontent">
		    
		        <img class="confirm-image" width="50" height="50" style=" margin-bottom: 1rem;" src="${pageContext.request.contextPath}/resourcesNew1/assets/status-check.svg">
		    
		    	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
		    	
		    	<div class="row" style="margin-bottom: 12px;">
		    		<div class=" col s6 m6 l6" style="">
		    				<label for="check-paid">
		    					<input type="radio" name="payment-status"
								 id="check-paid" value=""><span
								class="">Paid</span></label>
		    		</div>
	    			<div class=" col s6 m6 l6" style="">
	    					<label for="check-declined"><input type="radio" name="payment-status"
							 id="check-declined" value=""><span
							class="">Declined</span></label>
							</div>
		    	</div>
		    	</div>
		    	<div class="row">
		    	 	<div class="col s12 m12 l12" style="display: flex;">
		    	 			<textarea id="declinedreason" placeholder="Type Reason here.." name="declinedreason" rows="4" cols="50"  style="resize: none;"></textarea>
		    	 	</div>
		    	</div>
		    ...
		    <div class="modal-footer statusbtns" style="padding: 0% 5%; display: flex; flex:1; flex-direction: row; align-items: center; justify-content: space-evenly; background-color: #fff; border: none;">
		    	<button type="submit" class="btn btn-primary " id="confirmstatus" style="width: 30%; background-color: #005baa; color: #fff; height: 35px; font-size: 12px; border-radius: 50px !important; font-family: 'Poppins', sans-serif; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Confirm</button>
				<button type="button"  id="cancelBtn" class="btn btn-secondary modal-close " id="" style="width: 30%; background-color: #fff; color: #005baa; border: 1px solid #005baa; height: 35px; font-size: 12px; border-radius: 50px !important; font-family: 'Poppins', sans-serif; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Cancel</button>
...
function declineReason(){
	  ...
	  var elems = document.querySelector('#declineReasonModal');
...
function statusConfirm(){
	  ...
	  var elems = document.querySelector('#statusConfirmModal');
	  var instances  = M.Modal.init(elems, {
	        dismissible: false,
	        in_duration: 0
	});
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var paidRadio = document.getElementById("check-paid");
    var declinedRadio = document.getElementById("check-declined");
    var declinedReasonTextarea = document.getElementById("declinedreason");

    declinedReasonTextarea.style.display = "none";


    paidRadio.addEventListener("change", function () {
     
      if (paidRadio.checked) {
        declinedReasonTextarea.style.display = "block";
        declinedReasonTextarea.placeholder = "Type amount here...";
      }
    });

    declinedRadio.addEventListener("change", function () {

      if (declinedRadio.checked) {
        declinedReasonTextarea.style.display = "block";
        declinedReasonTextarea.placeholder = "Type Reason here...";
      }
    });
  });
</script>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var statusConfirmForm = document.getElementById("statusConfirmForm");
    var cancelBtn = document.getElementById("cancelBtn");

    cancelBtn.addEventListener("click", function () {
     
      statusConfirmForm.reset();
