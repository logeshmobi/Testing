	  	  ...
	  	  var e = document.getElementById("drop_search");
	      	...
	      	if (strUser == "Ref") {
	      		document.getElementsByName('search')[0].placeholder = 'Ex: SellerOrderNo';
	      		  ...
	      		  document.getElementsByName('search')[0].placeholder = 'Ex:Enter last 4 digits only';
	      	  }else if (strUser == "Ap_Code") {
	      		  document.getElementsByName('search')[0].placeholder = 'Ex:AID Response ID';
	      	  }else if (strUser == "RRN") {
				...
				<div class="card blue-bg text-white">
					<div class="card-content">
						<div class="d-flex align-items-center">
							<h3 class="text-white">
								<strong> EZYWAY Transaction Summary</strong>
							</h3>
						</div>
				...
				<div class="card border-radius">
					<div class="card-content padding-card">
						<div class="row">
							...
							<div class="input-field col s12 m3 l3">
								<input type="hidden" name="umezywaymid" id="umezywaymid"
									value=""> <select name="mid" id="mid"
									onchange="return loadDropDate14();">
									<option selected value="">Choose</option>

									<c:forEach items="${merchant1}" var="merchant1">
										<%-- <option value="${merchant1.mid.umEzywayMid}">
											${merchant1.businessName}~${merchant1.mid.umEzywayMid}</option> --%>
										<c:if test="${not empty merchant1.mid.umEzywayMid}">
											<option value="${merchant1.mid.umEzywayMid}">
											 ${merchant1.businessName}~${merchant1.mid.umEzywayMid}
											</option>
										</c:if>
										<c:if test="${not empty merchant1.mid.fiuuMid}">
											<option value="${merchant1.mid.fiuuMid}">
											 ${merchant1.businessName}~${merchant1.mid.fiuuMid}
											</option>
										</c:if>
									</c:forEach>
									<!-- <option value="MOTO">EZYPASS</option> -->
								</select> <label class="control-label">UMEZYWAY MID</label>
							</div>
						</div>
               ...
               <div class="card-content">
                      ...
                      <!-- SEARCH TEST -->
                        <div class="row" style="display:flex;align-items:center;justify-content:space-between;margin-left:01%;">
                           ...
                           <option value="Ap_Code">AID Response</option>
									...
									<c:forEach items="${paginationBean.itemList}" var="dto">
										<tr>
											...
											<td align="center"><c:if
													test="${dto.STATUS  =='SETTLED' || dto.STATUS  =='NOT SETTLED' ||dto.STATUS  =='CANCELLED' || dto.STATUS  =='VOIDED' }">
													<a href="javascript:void(0)" id="openNewWin"
														onclick="javascript: openNewWin('${dto.f263_MRN}')"> <i
											...
											<%-- <c:if
													...
													<a
														href="${pageContext.request.contextPath}/transaction/cancelPayment/${dto.f263_MRN}">
											...
											<td align="center">
												<%-- <c:if test="${dto.STATUS =='SETTLED' || dto.STATUS  =='NOT SETTLED'}"> --%>
												<%-- <c:if test="${dto.STATUS =='SETTLED'}">

													<input type="hidden" name="chBack" id="chBack"
														value="${dto.f007_TXNAMT}">
													<input type="hidden" name="mid" id="mid"
														value="${dto.f001_MID}">

													<button class="btn btn-primary" type="button"
														onclick="return loadChData(${dto.f007_TXNAMT},'${dto.f001_MID}','${dto.f263_MRN}');">
														<a><i class="material-icons">history</i></a>
													</button>

												</c:if> --%>
											</td>
											<td><c:if test="${dto.STATUS =='CHARGE BACK'}">${dto.CBDate} </c:if></td>
											<td><c:if test="${dto.STATUS =='CHARGE BACK'}">${dto.CBReason} </c:if></td>
											<td>${dto.submid}</td>
