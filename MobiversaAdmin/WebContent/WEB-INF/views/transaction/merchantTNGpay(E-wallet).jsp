...
function updateConfirmationMessage(amount) {	 
    document.getElementById("innerTextOfRefund").innerHTML = "Do you wish to proceed with the refund for this " + amount + "RM transaction?";
}
...
function initiateRefund(){
	 ...
	 /* 	console.log("Updatedamount : ",amount)
	    ...
	    var updated_amount = amount.replace(/,/g, '');
	    ...
	    $.ajax({
	        type: "GET",
	        url: "${pageContext.request.contextPath}/merchant/transaction/refund/tng",
	        data: {
	            "transactionId": rrn,
...
function closeForSuccessRefund(){
	...
	var url = "${pageContext.request.contextPath}/transactionUmweb/merchantTNGTransaction";
...
/* function openTngpayslip(rrn){
	...
	function openTngpayslip(date,time,txnamount,mid,rrn,authcode,reference,txntype,status,merchantName,mrn)
{
    ...
    document.getElementById("tng_slip_amount").innerText = txnamount;
    document.getElementById("tng_slip_amount_td").innerText = amountWithCurrency;
    document.getElementById("tng_slip_date").innerText = dateTimeString;
    document.getElementById("tng_slip_mid").innerText = mid;
    document.getElementById("tng_slip_mrn").innerText = mrn;
    document.getElementById("tng_slip_reference").innerText = reference; 
    document.getElementById("tng_slip_merchantname").innerText = merchantName; 

    // Analyze status and set appropriate message and color
    var statusElement = document.getElementById("tng_slip_Status");
    if (status === "SETTLED") {
...
<script lang="JavaScript">
	function loadSelectData() {
		...
		if (e == null ||e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/transactionUmweb/searchTNGpaymerchantlist?date='
	...
	function loadExpData() {
		...
		if (e == null ||e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/transactionUmweb/exportTNGpaymerchantlist?fromDate=' +fromdateString
...
<!--  -->
				...
				<div class="card blue-bg text-white">
					<div class="card-content">
						<div class="d-flex align-items-center">
							<h3 class="text-white">
								<strong>TNG Transaction Summary</strong>
							</h3>
						</div>
    ...
    <!-- no records found popup end --> 
    
    <!-- ==============================================refund popup's start ======================================= -->
		...
		<div class="modal fade bd-example-modal-xl pop-body"
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content "
					...
					<p class="pop-head"
						style="background-color: #f9f9f9; width: 100%; height: 50px; margin-bottom: 8px; color: #005baa; padding-top: 12px; border-bottom: 2px solid #ffa500; font-weight: 600; font-size: 18px; margin-bottom:8px;">Confirmation</p>
					<img id="popupalertimageforrefund"
						src="${pageContext.request.contextPath}/resourcesNew1/assets/clarity_alert-solid.svg"
						width="60px !important; height:60px !important;">
					<p id="innerTextOfRefund" style="font-size: 18px;padding:0 10%;font-weight:400;margin-bottom:0;color:#72777B;"></p>

 
					<div class="modal-footer" id="modalfooter" style="padding:0;">

						<!-- refund buttons  -->
						<div id="refundButtons"
							style="padding: 2% 10%; display: flex; flex-direction: row; align-items: center; justify-content: space-between;">
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content "
					...
					<img id="failediconforrefundcustomized"
						src="${pageContext.request.contextPath}/resourcesNew1/assets/FailureIcon.svg"
						width="60px !important; height:60px !important;">
					<p id="innerTextOfFailureRefundCustomized"
			...
			<section class="payout-slip-main-container poppins-regular"
				...
				<div class="slip-card" id="slip-card-id">
					...
					<div class="main-status poppins-semibold">
	             	...
	             	<p id="tng_slip_Status" class="status status-success">${dto.STATUS}</p>
						...
						<div class="status-container">
							...
							<p class="amount poppins-regular">
								MYR <span class="poppins-semibold amount-value"
									id="tng_slip_amount"></span>
							</p>
							<p class="time-stamp poppins-semibold" id="tng_slip_date"></p>
						...
						<table>
							<tr class="no_border_bottom">
								...
								<td class="poppins-medium xpay_slip_wordBreak"
									style="text-transform: uppercase;" id="tng_slip_merchantname"></td>
							</tr>
							<tr class="no_border_bottom">
								<th class="poppins-regular xpay_slip_whiteSpace">Order ID
									</th>
								<td class="poppins-medium xpay_slip_wordBreak"
									id="tng_slip_reference"></td>
							</tr>
							<tr class="no_border_bottom">
							...
							<tr class="no_border_bottom">
								...
								<td class="poppins-medium xpay_slip_wordBreak"
									style="text-transform: uppercase;" id="tng_slip_mid"></td>
							</tr>

							<tr class="no_border_bottom">
								<th class="poppins-regular xpay_slip_whiteSpace">Payment
									Method</th>
								<td class="poppins-medium xpay_slip_wordBreak">Touch'n Go</td>
							</tr>
...
<%--											<td>${dto.preauthfee}</td>--%>
												...
												<c:if test="${dto.cardType == 'TNG'}">
													<c:if
														...
														<a href="javascript:void(0)" id="openTngslip"
															onclick="javascript: openTngpayslip(
															'${dto.date}',
															'${dto.time}',
															'${dto.f007_TXNAMT}',
															'${dto.f001_MID}',
											...
											<td class='key_hover'><span
													...
													class="w24" src='data:image/png;base64,<%=eyeimg%> ' /> </span> <span
													onclick="hide_rrn('${id.index}')" id="show_rrn_${id.index}"
													class="hide_key">${dto.submerchantmid}</span></td>
									...
									<%-- 		<td style="text-align: center;"><c:if
													...
													<c:if test="${dto.STATUS =='NOT SETTLED' && dto.txnType !='AUTHSALE'}">
														<a
															...
															<img class="w24"
															src='data:image/png;base64,<%=voidimage%> ' />
														</a>
													</c:if>
												</c:if></td> --%>
											   ...
											   <fmt:parseDate var="formattedCurrentDate" value="${currentDate}" pattern="dd/MM/yyyy" />
												 ...
												 <c:if test="${settlementDate != null && settlementDate != 'undefined' && settlementDate != '' && settlementDate < formattedCurrentDate && dto.STATUS == 'SETTLED'}">
													<a href="javascript:void(0)"
														onclick="refundRequest('${dto.f007_TXNAMT}', '${dto.f011_AUTHIDRESP}')">
														<img class="w24"
