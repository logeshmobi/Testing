   ...
   // alert(newWindow);
	...
	function loadSelectData() {
		...
		if (e == null || e == '' || e1 == null || e1 == ''|| e3 == null||e3 == '') {
			...
			//alert("to date:" + document.getElementById("dateval2").value);
			 document.location.href = '${pageContext.request.contextPath}/transaction1/search1?date=' + fromdateString
			+ '&date1=' + todateString+ '&status=' + e3 + '&currPage=' + PageNumber; 
		...
		if((e == null || e == '') && (e1 == null || e1 == '') && (e3 == null || e3 == '')){
			...
			/* document.location.href = '/transaction1/search1?date=' + e
					+ '&date1=' + e1 +'&currPage='+pnum; */
					 document.getElementById("dateval1").value = e;
					document.getElementById("dateval2").value = e1;
			 document.location.href = '${pageContext.request.contextPath}/transaction1/search1?date=' + e
	...
	function loaddata() {
		...
		if(e == '' && e1 == '' && e3 == ''){
			...
			document.location.href = '${pageContext.request.contextPath}/transaction1/export?date=' + fromdateString
					...
					<tbody>
						<c:forEach items="${paginationBean.itemList}" var="dto">
							<tr>
								...
								dto.status =='CASH SALE' || dto.status =='CASH CANCELLED' || dto.status =='NOT SETTLED' ||
								...
								<a href="javascript:void(0)" id="openNewWin"
										...
										<td align="center"><c:if
												test="${dto.status =='NOT SETTLED' &&  dto.txnType =='EZYMOTO'}">
												<a
													href="${pageContext.request.contextPath}/transaction/motoCancelPayment/${dto.trxId}">
													<i class="material-icons">create</i></a>
											</c:if>
											<c:if
												test="${dto.status =='NOT SETTLED' && dto.txnType =='EZYLINK'}">
												<a
													href="${pageContext.request.contextPath}/transaction/motoLinkCancelPayment/${dto.trxId}">
													<i class="material-icons">create</i></a>
											</c:if>
											</td>

								<td>
								<c:if test="${dto.status =='NOT SETTLED' && (dto.txnType =='EZYWAY' || dto.txnType =='EZYMOTO')}">
											
											<button class="btn btn-primary" id="buttonSub" type="submit"
				onclick="return cal('B','${dto.trxId}');">TO BLOCK</button>
								
								</c:if>
								
								
								<c:if test="${dto.status =='BLOCKED' && (dto.txnType =='EZYWAY' || dto.txnType =='EZYMOTO')}">
											
											<button class="btn btn-primary" id="buttonSub" type="submit"
				onclick="return cal('A','${dto.trxId}');">UNBLOCK</button>
