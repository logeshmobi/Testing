...
.choose .select-wrapper input.select-dropdown{
...
function loadExpData() {
	var chooseType = document.getElementById("allMerchants").value;
	var businessName = document.getElementById("merchantName").value;
    ...
    // Validate chooseType (either 'All' or 'Merchant')
    if (!chooseType) {
        alert("Please select Choose Type");
        return false;
    }
    
    // If 'Merchant' is selected, check for Business Name
    if (chooseType === "Merchant" && !businessName) {
        alert("Please select a Business Name");
        return false;
    }
    ...
    // Check the date range condition based on Choose Type
    if (chooseType === "All") {
        if (diffDays > 7) {
            alert("Please select a date range of 7 days or less for 'All Merchants'.");
            return false;
        }
    } else if (chooseType === "Merchant") {
        if (diffDays > 31) {
            alert("Please select a date range of 31 days or less for 'Merchant'.");
            return false;
        }
    }

    // If all validations pass, display the modal
				...
				<div class="card blue-bg text-white">
					<div class="card-content" style="">
						<div class="row"
							...
							<%-- merchant choose --%>
							<div class="choose input-field col s12 m4 l3" style="margin-left: 0;">
								<select id="allMerchants" name="export">
									<option value="" selected>Choose</option>
									<option value="All">All</option>
									<option value="Merchant">Merchant</option>
								</select> <label for="Choose Type">Choose Type</label>
							</div>
							...
							<div class="col s12 m4 l4 input-field businessname"
								...
								<label for="merchantName" id="label_businessname">Select
									Business Name</label> <select name="merchantName" id="merchantName"
				...
				<div class="card border-radius">
					<div class="card-content padding-card">
						<div class="row">
							...
							<!-- Export Type -->
							<div class="choose input-field col s12 m3 l3">
	...
	document.addEventListener('DOMContentLoaded', function () {
	   ...
	   function submitEmail() {
		 //  $("#overlay").show();
		    ...
		    var businessName = document.getElementById("merchantName").value;
		    var type = document.getElementById("allMerchants").value; 
...
/* 
		     ...
		     */
		     if (type === 'All') {
		    	 $("#overlay").show();
			        document.location.href = '${pageContext.request.contextPath}/transaction/exportAllPayoutTxnAsEmail?date=' + fromDateString + '&date1=' + toDateString + '&emailId=' + email + '&export=' + exportType;
...
.select-search-hidden .select2-container {
    ...
    var initialOverflow = body.style.overflow;
		...
		<script>
    document.getElementById('allMerchants').addEventListener('change', function () {
        const businessNameDropdown = document.getElementById('merchantName');
        const Choosetype = document.getElementById('allMerchants').value;
        if (Choosetype== 'All') {
            businessNameDropdown.disabled = true;
            businessNameDropdown.value = ''; // Clear selected value
        } else {
            businessNameDropdown.disabled = false;
        }
    });
</script>
		...
		<script type="text/javascript">
                ...
                // Optional: Initialize any other JS components, like select2, if needed
                $(document).ready(function () {
                    $("#merchantName").select2(); // If you are using select2 for enhanced dropdowns
                });
            </script>
            
             <%--            business name select style--%>
            <style>
                #label_businessname {
                    position: static !important;
                    color: #858585;
