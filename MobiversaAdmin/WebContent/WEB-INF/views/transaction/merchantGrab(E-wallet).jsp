...
function initiateRefund(){
	 ...
	 /* 	console.log("Updatedamount : ",amount)
	    ...
	    $("#overlay_gifloader").show();
	      ...
	      $.ajax({
	        type: "GET",
	        url: "${pageContext.request.contextPath}/merchant/transaction/refund/grabpay",
	        data: {
	            "transactionId": rrn,
...
function closeForSuccessRefund(){
	...
	var url = "${pageContext.request.contextPath}/transactionUmweb/MerchantGrab";
...
function openGrabpayslip(date,time,txnamount,mid,rrn,authcode,reference,txntype,status,merchantName)
{
    ...
    document.getElementById("grab_slip_amount").innerText = txnamount;
    document.getElementById("grab_slip_amount_td").innerText = amountWithCurrency;
    document.getElementById("grab_slip_date").innerText = dateTimeString;
    document.getElementById("grab_slip_mid").innerText = mid;
    document.getElementById("grab_slip_rrn").innerText = rrn;
    document.getElementById("grab_slip_reference").innerText = reference; 
    document.getElementById("grab_slip_merchantname").innerText = merchantName; 

    // Analyze status and set appropriate message and color
    var statusElement = document.getElementById("grab_slip_Status");
    if (status === "SETTLED") {
...
<script lang="JavaScript">
	function loadSelectData() {
		...
		if (e == null ||e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/transactionUmweb/searchGrabTransactionlist?date='
...
/* SEARCH IMPLEMENTATION */
	...
	//	var txnType = document.getElementById("txnType").value;
		...
		if (e == null ||e == '' || e1 == null || e1 == '') {
			...
			/* document.location.href = '${pageContext.request.contextPath}/transaction/searchUMEzyway?date='
					+ e + '&date1=' + e1 + '&txnType=' + e2; */
			document.location.href = '${pageContext.request.contextPath}/transactionUmweb/exportGrabTransactionlist?fromDate=' +fromdateString
				...
				<div class="card blue-bg text-white">
					<div class="card-content">
						<div class="d-flex align-items-center">
							<h3 class="text-white">
								<strong>GRAB Transaction Summary</strong>
							</h3>
						</div>
		...
		<div class="modal fade bd-example-modal-xl pop-body"
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content "
					style="padding: 0 !important; margin: 0 !important;">
					<p class="pop-head"
						style="background-color: #f9f9f9; width: 100%; height: 50px; margin-bottom: 8px; color: #005baa; padding-top: 12px; border-bottom: 2px solid #ffa500; font-weight: 600; font-size: 18px;">Confirmation</p>
			...
			<section class="payout-slip-main-container poppins-regular"
				...
				<div class="slip-card" id="slip-card-id">
					...
					<div class="main-status poppins-semibold">
	             	...
	             	<p id="grab_slip_Status" class="status status-success">${dto.STATUS}</p>
						...
						<div class="status-container">
							...
							<p class="amount poppins-regular">
								MYR <span class="poppins-semibold amount-value"
									id="grab_slip_amount"></span>
							</p>
							<p class="time-stamp poppins-semibold" id="grab_slip_date"></p>
						...
						<table>
							<tr class="no_border_bottom">
								...
								<td class="poppins-medium xpay_slip_wordBreak"
									style="text-transform: uppercase;" id="grab_slip_merchantname"></td>
							</tr>
							<tr class="no_border_bottom">
								<th class="poppins-regular xpay_slip_whiteSpace">Order ID
									</th>
								<td class="poppins-medium xpay_slip_wordBreak"
									id="grab_slip_reference"></td>
							</tr>
							...
							<tr class="no_border_bottom">
								...
								<td class="poppins-medium xpay_slip_wordBreak"
									style="text-transform: uppercase;" id="grab_slip_mid"></td>
							</tr>
							
							<tr class="no_border_bottom">
								<th class="poppins-regular xpay_slip_whiteSpace">Payment
									Method</th>
								<td class="poppins-medium xpay_slip_wordBreak">GrabPay</td>
							</tr>
...
<%--											<td>${dto.preauthfee}</td>--%>
											...
											<td align="center"><c:if
													...
													<c:if
														...
														<a href="javascript:void(0)" id="openGrabpayslip"
															onclick="javascript: openGrabpayslip(
															'${dto.date}',
															'${dto.time}',
															'${dto.f007_TXNAMT}',
															'${dto.f001_MID}',
										...
										<%-- 	<td style="text-align: center;"><c:if
													test="${dto.cardType == 'MASTERCARD DEBIT' || dto.cardType == 'MASTERCARD CREDIT' || dto.cardType == 'VISA DEBIT' || dto.cardType == 'VISA CREDIT' }">
													<c:if
														test="${dto.STATUS =='NOT SETTLED' && dto.txnType !='AUTHSALE'}">
														<a
