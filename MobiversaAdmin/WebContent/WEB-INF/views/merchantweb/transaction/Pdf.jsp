 ...
 */
		...
		if (e == '' && e1 == '' && e2 == '') {
			...
			//alert("from date:"+ e);
			//alert("todate:"+ e1);
			document.form1.action = "${pageContext.request.contextPath}/transactionweb/pdfsearch";
			//form.submit;
	...
	function loadData(num) {
		...
		/* if (e == null || e == '' ) {
			...
			//alert("else : "+e+" "+e1);
			document.location.href = '${pageContext.request.contextPath}/transactionweb/pdfsearch?fromDate='
					+ e + '&toDate=' + e1 + '&currPage=' + pnum + '&mid=' + e2;
	...
	function loaddata() {
		...
		if (e == null || e == '' || e1 == null || e1 == '') {
			...
			//alert(e);
			document.form1.action = "${pageContext.request.contextPath}/transactionweb/exportCashTrans";
			//form.submit();
			document.form1.submit();
...
function pdf(name){
	...
	var src = document.getElementById('popOutiFrame').src;
	 src=url+'transactionweb/getpdf/'+name;
							...
							<div class="row">
								...
								<div class="input-field col s12 m3 l3">
									<input type="hidden" name="tid1" id="tid1"
										<c:out value="${midlist}"/>> <select name="mid" id="mid"
										onchange="return loadDropDate();">
										<option selected value=""><c:out value="MID" /></option>
										<c:forEach items="${midlist}" var="t">

											<option value="${t}">${t}</option>

										</c:forEach>
									</select> <label for="tid">MID </label>
						...
						<div class="card-content padding-card">
							...
							<div class="table-responsive m-b-20 m-t-15">
								...
								<table id="data_list_table"
									...
									<tbody>
										<c:forEach items="${paginationBean.itemListt}" var="dto" varStatus="count">
											<tr>

												<td style="font-size:20px;">${paginationBean.filenamelist[(count.index)]}</td>


												<td><a href="javascript:void(0)" id="openNewWin"
													onclick="javascript: pdf('${paginationBean.itemListt[count.index]}')"> 
													<img class="w24" src='data:image/png;base64,<%=actionimg%> '/>
														</a>
