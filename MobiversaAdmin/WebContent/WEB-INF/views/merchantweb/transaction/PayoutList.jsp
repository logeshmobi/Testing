...
<script lang="JavaScript">
	...
	<%--	var searchInput = document.getElementById('searchApi').value;--%>

	<%--	var searchInputWithQuotes = "'" + Value + "'";--%>

	<%--	/*  var dropdown = document.getElementById("drop_search");--%>
	...
	<%--	}--%>
	<%--	/*  if(selectedValue === ""){--%>
	<%--		 alert('Please select a Choose type.');--%>
	<%--		 $("#overlay").hide();--%>
	<%--            return;--%>
	<%--	 } */--%>

	<%--	document.location.href = '${pageContext.request.contextPath}/searchNew/searchPayoutId?VALUE='--%>
	<%--			+ searchInputWithQuotes;--%>
	<%--	form.submit;--%>
	...
	function payoutIdSearch() {
		...
		document.location.href = '${pageContext.request.contextPath}/searchNew/searchPayoutId?VALUE=' + searchInput + '&transaction_type='
				+ Value;
...
<style>
.txn-details-option {text - wrap:nowrap;
	
}

.slip-table tr td {font - size:14px;
...
<body class="">
	...
	<div class="container-fluid">
     ...
     -->
		...
		<%--							<div class="input-field col s12 m3 l3" style="margin-left:07%;width:30%;">--%>
		<%--								<input type="text" id="searchApi" name="search" class="" style="font-family:'Poppins',sans-serif;" placeholder="Please select type to search"  oninput="validateAmount()">--%>
		<%--							</div>--%>
		<%--							<div class="input-field col s12 m3 l3" style="width:10%;margin-left:07%;">--%>
		<%--								<div class="button-class" style="float:left;" >--%>
		<%--									<button class="btn btn-primary blue-btn" type="button" onclick="payoutIdSearch()" style="font-family:'Poppins',sans-serif;width:100%;font-size:14px;">Search</button>--%>
		<%--								</div>--%>
		<%--							</div>--%>
		<%--						</div>--%>
		<%--					</div>--%>
		...
		<%--											&lt;%&ndash; <td>${dto.payoutstatus} -&nbsp; ${dto.paymentreason}</td> &ndash;%&gt;--%>
		<%--										</c:when>--%>
		<%--										<c:otherwise>--%>
		<%--											<td>${dto.payoutstatus}</td>--%>
		<%--										</c:otherwise>--%>
		...
		<%--										&lt;%&ndash; <td style="text-align: center;"><c:if--%>
				...
				<div class="card border-radius">
					<div class="card-content padding-card">
						...
						<div class="table-responsive m-b-20 m-t-15" id="page-table">
							<table id="data_list_table"
								class="table table-striped summary-table table-bordered">
								<thead>
									<tr>
										...
										<th style="text-align: center; font-size: 14px">Decline
											Reason</th>
								...
								<tbody>
									...
									<c:forEach items="${paginationBean.itemList}" var="dto">
										...
										<tr>
											...
											<td style="text-align: left; font-size: 14px">${dto.payeeaccnumber}</td>
											...
											<c:choose>
												...
												<c:when test="${dto.payoutstatus == 'Rejected'}">
													<td
															style="text-align: left; display: flex; align-items: center; justify-content: flex-start; font-size: 14px; margin-top: 5px">
														<img class=""
															 src="${pageContext.request.contextPath}/resourcesNew1/assets/Declined.svg"
															 width="20" height="20" style="margin-right: 5px;" /> <span
															style="color: red; font-weight: 450;">Rejected</span>
													</td>
												</c:when>
                                            ...
                                            <td
                                                ...
                                                <img class=""
                                                     ...
                                                     width="20" height="20" style="margin-right: 5px;"/> <span
                                                    style="color: #8A8A8A; font-weight: 500;">In Process</span>
                                            </td>
                                        </c:when>
											...
											<td style="text-align: center;"><c:if
													...
													<a href="javascript:void(0)" id="openPayoutslip"
															<%-- onclick="javascript: openDeclinedReason('${dto.failurereason.contains('ERROR (') || dto.failurereason.contains('Exception') || dto.failurereason.contains('exception')? 'Host Side Error' : dto.failurereason}')"> --%>
												...
												<c:if test="${dto.payoutstatus == 'Rejected'}">
													<a href="javascript:void(0)" id="openPayoutslip"
													   onclick="javascript: openPendingReason('Account Name mismatch')">
														<div style="padding-top: 5px;">
															<img class="" width="20" height="20"
																 src="${pageContext.request.contextPath}/resourcesNew1/assets/blureye.svg" />
														</div>
...
<%-- <c:if test="${xpayId != '2' }">
                                   ...
                                   <%--  <c:if test="${xpayId == '2' }"> --%>
                                    <td style="text-align: center; padding-top: 8px;"><c:if
                                        ...
                                        <a href="javascript:void(0)" id="openPayoutslip "
                                           onclick="javascript: openPayoutslipXpay(
                                                    '${dto.invoiceidproof}',
                                                   '${dto.payeename}',
                                                   '${dto.createdby}',
                                                   '${dto.payoutamount}',
	...
	<div id="slip-modal-id" class="slip-modal-class">
		<div id="slip-modal-content-id" class="slip-modal-content-class">
			<div style="padding: 0 10px;">
				...
				<div class="content" style="padding: 20px;display: flex;justify-content: center;">
...
//		         Pagination.code += '<a onclick="dynamic(' + lastPage + ')">' + lastPage + '</a>';
    ...
    * * * * * * * * * * * * * * * * */
	...
	function openPayoutslipMore(payoutDate, merchantName, payeeName, payeeBrn, payeeAccNumber, payeeBankName, invoiceIdProof, payoutId, payoutamount, payoutStatus, timeStamp, paidTime, paidDate, submerchantMid, businessName, failureReason, createdby, payouttype, srcrefno,xpayId,paymentReference) {
...
/* 
        if (payoutStatus === 'Paid' && xpayId !=='2') {
            var salesSlipLink = table_content.querySelector(".openPayoutslipInPopup");
            salesSlipLink.addEventListener("click", function() {
                openPayoutslip(invoiceIdProof, payeeName, createdby, payoutamount, timeStamp.replace(".0", ""), srcrefno, payoutStatus, payouttype, payoutDate);
            });
        }
 */
        if (payoutStatus === 'Paid') {
            var salesSlipLink = table_content.querySelector(".openPayoutslipInPopup");
            salesSlipLink.addEventListener("click", function() {
                openPayoutslipXpay(invoiceIdProof, payeeName, createdby, payoutamount, timeStamp.replace(".0", ""),srcrefno, payoutStatus, payouttype, payoutDate,paymentReference,payoutId,paidDate,paidTime);
            });
        }
	...
	function openPayoutslip(invoiceIdProof, payeeName, createdBy, payoutAmount, timeStamp, srcrefno, payoutStatus, payoutType,payoutdate) {
		console.log("timestamp from paramter "+timeStamp);
		var modal = document.getElementById("slip-modal-id");
		modal.style.display = "block";

		console.log("src ref no is ",srcrefno);

		var slipCard = document.querySelector(".slip-card");
		slipCard.scrollIntoView({block: 'end' });


		var formattedPayeeName = "Hi, " + payeeName + ",";
		var formattedPayoutToPayeeName = "Payout to " + payeeName;
		var formattedPayoutMerchantName= createdBy +" "+"has made payment of";
		var formattedPayoutDate= "to you on"+" "+payoutdate.replace(".0", "")+".";
		// var formattedPayoutDate= "to you on"+" "+timeStamp.replace(".0", "")+".";
		var topAmount="RM "+payoutAmount;
		var formatedPayoutAmountValue="RM "+payoutAmount;

		document.getElementById("payoutMerchantName").innerText = formattedPayoutMerchantName;
		document.getElementById("payoutToId").innerText = formattedPayoutToPayeeName;
		document.getElementById("payoutPayeeValue").innerText = formattedPayeeName;
		document.getElementById("srcrefnoValues").innerText =srcrefno;
		document.getElementById("payoutPayeeValue").innerText = formattedPayeeName;
		document.getElementById("payoutAmountValue").innerText = formatedPayoutAmountValue;
		document.getElementById("invoice-id").innerText = invoiceIdProof;
		document.getElementById("payout-status").innerText = payoutStatus;
		document.getElementById("payout-type").innerText = payoutType;
		document.getElementById("top-amount").innerText = topAmount;


		var createdDate =timeStamp.replace(".0", "");
		var formattedDate = formatDate(createdDate);
		document.getElementById("top-date").innerText = formattedDate;
		console.log(formattedDate);



		document.getElementById("payout-date").innerText = formattedPayoutDate;

		// console.log("Invoice ID Proof: ", invoiceIdProof);
	...
	// document.getElementById("slip-modal-close-btn-id").addEventListener("click", function() {
	// 	closeSlipModal();
	// });
	...
	function closeSlipModal() {
	  ...
	  function payoutDateNewSlip(dateString) {
	        // Split the date string into day, month, and year
	        let dateParts = dateString.split('/');
	        let day = parseInt(dateParts[0], 10);
	        let month = parseInt(dateParts[1], 10) - 1; // Month is 0-based in JavaScript Date
	        let year = parseInt(dateParts[2], 10);

	        // Create date objects for the incoming date, today, and yesterday
	        ...
	        // Compare the input date with today's and yesterday's dates
  ...
  function openPayoutslipXpay(invoiceIdProof, payeeName, createdBy, payoutAmount, timeStamp, srcrefno, payoutStatus, payoutType, payoutdate,paymentReference,payoutId,paidDate,paidTime) {

  	/* console.log("coming is :"+paymentReference) */
      body.style.overflow = "hidden";
      var modal = document.getElementById("xPay_slip-modal-id");
      var modal1 = document.getElementById("payout-slip-main-container-id");
      var modal2 = document.getElementById("slip-card-id");
     ...
     // document.getElementById("new_slip_timestamp").innerText = formattedDate;
      document.getElementById("new_slip_invoiceIdProof").innerText = invoiceIdProof;
   //   document.getElementById("new_slip_referenceNo").innerText = srcrefno;
   ...
   //   document.getElementById("new_slip_when").innerText = payoutDateNewSlip(payoutdate);
     ...
     /*  console.log("sjhadkajdas "+payoutDateNewSlip(payoutdate)); */
      ...
      document.getElementById("new_slip_paymentReference").innerText = paymentReference === "null" ? "" : paymentReference;
      document.getElementById("paidTime").innerText = paidTime;
      document.getElementById("paidDate").innerText = formatPayoutDate(paidDate);
...
<!-- new design for payslip payout end -->
