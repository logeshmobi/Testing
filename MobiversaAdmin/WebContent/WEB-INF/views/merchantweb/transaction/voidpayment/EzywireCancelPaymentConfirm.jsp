...
<script type="text/javascript">
	function cancel() {
		...
		document.getElementById('form1').action = '${pageContext.request.contextPath}/transactionweb/listcard';
		document.getElementById('form1').submit();
	}
</script>
<body>
	<form:form method="post" id="form1" name="form1" commandName="txnDet"
		action="${pageContext.request.contextPath}/transactionUmweb/EzywireCancelPaymentByMerchant">
					...
					<div class="card border-radius">
						<div class="card-content padding-card">
							...
							<div class="row">
								...
								<form:input type="hidden" value="${txnDet.merchantId}"
									path="merchantId" />
								<form:input type="hidden" value="${txnDet.mid}" path="mid" />
								<form:input type="hidden" value="${txnDet.expectedDate}"
									path="expectedDate" />
								<form:input type="hidden" value="${txnDet.amount}" path="amount" />
								<form:input type="hidden" value="${txnDet.contactName}"
									path="contactName" />
								<form:input type="hidden" value="${txnDet.pan}" path="pan" />
								<form:input type="hidden" value="${txnDet.apprCode}"
									path="apprCode" />
								<div class="d-flex align-items-center">
									<h3 class="card-title">EZYWIRE Transaction Details</h3>
								</div>
								<table class="table table-striped" width="100%">
									<tbody>
										...
										<tr>
											<td><label class="control-label">Card Holder
													Name</label></td>
							...
							<div class="row">
								<div class="input-field col s12 m6 l6 ">
									...
									<button type="submit" class="btn btn-primary">
										<a
											href="${pageContext.request.contextPath}/transactionUmweb/alllist/1"
