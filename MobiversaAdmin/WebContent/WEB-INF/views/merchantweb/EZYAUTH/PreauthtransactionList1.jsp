...
<%
	...
	String saleimg = resource.getString("NEWSALE");
%>
...
<script lang="JavaScript">
	function loadSelectData() {
		...
		if (e == null ||e == '' || e1 == null || e1 == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/merchantpreauth/searchPreAuth1?date='
	...
	function loadExpData() {
		...
		if (e == null || e == '' || e1 == null || e1 == '') {
			...
			/* document.location.href = '${pageContext.request.contextPath}/transaction/searchUMEzyway?date='
					+ e + '&date1=' + e1 + '&txnType=' + e2; */
			document.location.href = '${pageContext.request.contextPath}/merchantpreauth/ExportPreAuth1?fromDate=' + fromdateString
...
function loaddata() {
	...
	if (e == null || e1 == null || e == '' || e1 == '') {
		...
		//alert("test2: " + e + " " + e1);
		 document.location.href = '${pageContext.request.contextPath}/merchantpreauth/ExportPreAuth1?fromDate=' + e
				+ '&toDate=' + e1 +'&export='+e5; 
		//alert(e);
...
document.getElementById('hide_rrn_'+id).style.display ="block";
		...
		 
		<script>
    function openDialog(transactionAmount,transactionid, hostType) {
	
      document.getElementById("popup").style.display = "block";
      $("#overlay-popup").show();
      
      document.getElementById("transactionAmount").value = transactionAmount;
      document.getElementById("hostType").value = hostType;
      
    //  console.log("host type : ",hostType);
      
      document.getElementById("transactionid").value = transactionid;
      document.getElementById("preauthamt").innerHTML = transactionAmount;
...
    }
    ...
    function ProcessCTS()
    {
         ...
         var txnid = document.getElementById("transactionid").value
         
         var hostType = document.getElementById("hostType").value
         
         if (document.getElementById("transactionAmount").checked) {
        	 
        	  var amt = document.getElementById("transactionAmount").value;
        	  var amount=amt;
        	  //alert("amount");
         ...
         if (document.getElementById("userAmount").checked) {
        	 
        	 var userAmt = document.getElementById("finalamount").value;
        
        	 var userAmount = userAmt;
            // var userAmount = parseFloat(userAmt.replace(/,/g, '')); 
            
           /*  var userAmount = parseFloat(userAmt); */
         ...
         if (hostType === 'FIUU') {
        	 urlString = "${pageContext.request.contextPath}/fiuuApi/fiuuConvertToSale"; 
         } else {
        	 urlString = "${pageContext.request.contextPath}/merchantpreauth/ConvertToSale";
         }
         ...
         $.ajax({
   			...
   			async: false,
   			type: "GET",
   		//	url: "${pageContext.request.contextPath}/merchantpreauth/ConvertToSale",
   			url: urlString,
   			data: {
   				"amount": amount,
...
$(document).ready(function() {
						...
						<div class="table-responsive m-b-20 m-t-15" id="page-table">
							<table id="data_list_table1"
                                ...
                                <tr>
                                    ...
                                    <td>${dto.status}</td>
                                    <td class='key_hover'>
                                        <span onclick="show_key('${id.index}')" id="hide_key_${id.index}">
                                            <img class="w24" src='data:image/png;base64,<%=eyeimg%>'/>
                                        </span>
                                        <span onclick="hide_key('${id.index}')" id="show_key_${id.index}"
                                              class="hide_key">${dto.approvalCode}</span>
                                    </td>
                                    <td class='key_hover'>
                                        <span onclick="show_rrn('${id.index}')" id="hide_rrn_${id.index}">
                                            <img class="w24" src='data:image/png;base64,<%=eyeimg%>'/>
                                        </span>
                                        <span onclick="hide_rrn('${id.index}')" id="show_rrn_${id.index}"
                                              class="hide_key">${dto.reference}</span>
                                    </td>
                                    <td>
                                        <c:if test="${dto.status != 'EXPIRED'}">
                                            <img onclick="openDialog('${dto.amount}','${dto.txnid}','${dto.serviceId}')"
                                                 class="w24"
                                                 src='data:image/png;base64,<%=saleimg%>'/>
                                        </c:if>
                                    </td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${dto.txntype == 'MOTO' && dto.status != 'EXPIRED'}">
                                                <a href="${pageContext.request.contextPath}/merchantpreauth/PreAuthVoid1/${dto.txnid}"
                                            ...
                                            <c:otherwise>
                                                <c:if test="${dto.status != 'EXPIRED'}">
                                                    <a href="${pageContext.request.contextPath}/merchantpreauth/PreAuthWireVoid1/${dto.txnid}"
                                                       style="text-decoration: none;">
                                                        <img class="w24" src='data:image/png;base64,<%=voidimg%>'/>
                                                    </a>
                                                </c:if>
	...
	<div id="popup" style="display: none;">
		<div class="popup-content"
			style="background-color: white; text-align: start; padding: 2rem; max-width: 25%; width: 100%; margin: 5rem; border-radius: 1rem; display: bl; box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
			<div class="popup-title" style="font-size: x-large;">Confirm
				Sale Amount</div>
			<hr>

			<div class="input-field col s12 m6 l6 ">
				<div class="lable" style="margin-bottom: 5px; margin-top: 15px;">
				</div>
				<p>
					<label><input type="radio" checked="checked"
						name="preauth_amt" id="transactionAmount" value="" onclick="toggleAmountInput('preAuth')"><span
						class="c969696">PreAuth Amount</span></label>
				</p>
			...
			<div>
				...
				<p style="text-align: center; color: #005baa; font-weight: bold;">
					RM <span id=preauthamt></span>
				</p>
			</div>
			...
			<div class="input-field col s12 m6 l6 ">
				...
				<p>
					<label><input type="radio" name="preauth_amt"
						id="userAmount" value="" onclick="toggleAmountInput('newAmount')"><span class="c969696">New
							Amount</span></label>
				</p>
			...
			<div>
				...
				<input type="text" placeholder="0.00" class="form-control"
					id='finalamount' disabled
					style="border: #005baa !important; border: solid 3px #005baa !important; border-radius: 25px;">
			</div>
			...
			<!-- service id  -->
			<input type="hidden" id="hostType" value="">

			<div class="popoup-footer" style="margin-top: 2rem; text-align: end;">
				<button
					style="min-width: 50px; background-color: #005baa; border-color: transparent; color: white; padding: 0.4rem; border-radius: 5px;"
					onclick="ProcessCTS()">Confirm</button>
				<button
					style="cursor: auto; min-width: 50px; background-color: #005baa; border-color: transparent; color: white; padding: 0.4rem; border-radius: 5px;"
      ...
      function toggleAmountInput(type) {
    	    ...
    	    if (type === 'newAmount') {
    	      ...
    	      finalAmountInput.addEventListener('input', function(){
    	    	  const amountValue = finalAmountInput.value.replace(/[^0-9]/g, '');
                  var formatedvalue = (parseFloat(amountValue) / 100).toFixed(2);
                  finalAmountInput.value = formatedvalue;
    	      });
    	    } else {
