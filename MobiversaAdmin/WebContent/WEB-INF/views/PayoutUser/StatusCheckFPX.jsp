...
<body>
	...
	<div class="container-fluid mb-0" id="main-container">
		...
		<div class="modal fade bd-example-modal-xl pop-body"
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content "
					...
					<p id="innerText"
						style="font-size: 20px; margin-bottom: 1px; color: black; z-index: 3;"></p>
					<p id="innerTextHelp"
				...
				<div class="card blue-bg text-white">
					<div class="card-content status-check-cardcontent">
						<div class="d-flex lign-items-center">
							<h3 class="text-white">
								<strong>Check FPX Transaction Status</strong>
							</h3>
						</div>
						...
						<div class="row sellerorderfield">
							...
							<div class=" col s12 m12 l12 ">
								<br>
								<form class="d-flex"
									style="flex-direction: row; align-items: flex-end; justify-content: flex-start;"
									action="${pageContext.request.contextPath}/payoutDataUser/getStatusFPX"
									method="post">
									...
									<div class="d-flex"
										style="flex-direction: column; width: 55%; padding-right: 8%">
										<label for="sellerOrderNum" class="sellerordertext">Seller
											Order Number</label> <input type="text" placeholder="0000000000"
											name="sellorOrderNum" id="sellerorderno">
									</div>
		...
		<script>
        document.querySelector('.statuscheckbtn').addEventListener('click', function(event) {
										...
										<tbody class="responsedata">
											...
											<c:forEach items="${response.itemList}" var="dto"
												...
												<tr class="dto-${loop.index}">
													...
													<td>${dto.responseData.status}</td>
													<td>${dto.responseData.transactionDate}</td>
													<td>${dto.responseData.failureReason}</td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
	...
	const form = document.querySelector('form');
	  ...
	  form.addEventListener('submit', (event) => {
	    const invoiceIdInput = document.querySelector('#sellerorderno');

	    if (invoiceIdInput.value === '') {
	      event.preventDefault();
	      alert('Please Enter Seller Order Number.');
...
/* 				console.log("${response.itemList.get(0).responseCode}","${response.itemList.get(0).responseMessage}"); 
 */			
			     if(${empty response.itemList.get(0).responseData}){
				         ...
				         document.getElementById("innerText").innerText = "${response.itemList[0].responseMessage}";
