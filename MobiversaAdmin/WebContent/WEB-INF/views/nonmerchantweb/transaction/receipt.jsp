<%@page	import="com.mobiversa.payment.controller.MerchantWebTransactionController"%>
<%@page import="com.mobiversa.common.bo.Merchant"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<%@taglib prefix="sec"
	uri="http://www.springframework.org/security/tags"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>

<html>
<head>
<meta charset="UTF-8">
<meta
	content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"
	name="viewport">
	<script src="/resourcesNew/js/jQuery-2.1.4.min.js" type="text/javascript"></script>
	<script src="${pageContext.request.contextPath}/resourcesNew/assets/js/jquery-1.11.3.min.js"></script>
<link href="/resourcesNew/bootstrap/css/bootstrap.min.css"
	rel="stylesheet" type="text/css" />
<link href="/resourcesNew/css/AdminLTE.min.css" rel="stylesheet"
	type="text/css" />
<style>
@media only screen and (max-width: 690px) { "
	.container {
		width: 100% !important;
	}
	"
}

"
.flexibleImage { "
	max-width: 100px !important;
	"
	width
	100%
	!important;
	"
}
"
</style>
<script lang="JavaScript">

function displaySign() {
	var sign = document.getElementById("sign").value;
	//alert("Data :"+sign);
	document.getElementById("nsign").style.display='none';
	document.getElementById("tsign").style.display='none';
	//alert("Data :"+sign);
	if(sign == null || sign == ''){
		//alert("if :"+sign);
		document.getElementById("nsign").style.display='';
	}else{
		//alert("else :"+sign);
		document.getElementById("tsign").style.display='';
		
	}
}

</script>
</head>
<body style="font-family: 'Oswald', sans-serif; color: #666;" onload="displaySign();">
	<table class="container" width="100%">
		<table align="center" width="400"
			style="margin: 20px auto; background: #fff; border: 3px solid #ccc; border-style: solid; padding: 10px;">
			<tr>
				<td align="center">
					<!--  <div style="margin-left:278px"> <img
					src="/resourcesNew/img/mobiversa logo-1.png">
					style="margin-left: 180px"> --> <img
					src="data:image/jpg;base64," />

				</td>
			</tr>
			<tr>
				<td>
				
				  <table cellpadding="0" cellspacing="0" align = "center" style="font-size:16px;">
  <tr>
  <td style="padding:5px;text-align:center;font-size:16px;display:block;font-family:Lao UI;">${dto.merchantName}</td>
  </tr>
  <tr>
  <%-- <td style="padding:5px;text-align:center;font-size:10px;font-family:Lao UI;">
  ${dto.merchantAddr1}<br/>
${dto.merchantCity}, ${dto.merchantPostCode}<br/>
  ${dto.merchantContNo}
  </td> --%>
  
  <td align="center"
								style="padding: 5px; text-align: center; font-size: 15px; font-family:Helvetica;color:#175da9;">
								<b>${dto.merchantName}</b><BR/>
								${dto.merchantAddr1}<br /> ${dto.merchantPostCode},${dto.merchantCity},
								<br /> ${dto.merchantState} <br /> 
								Phone: ${dto.merchantContNo}
							</td>
  
  </tr>
  </table>
				
					<h3
						style="font-size: 20px; color: #0b9bff; display: block; text-align: center; padding: 10px; border-top: 1px solid #ccc; font-family: Lao UI;">
						<u>${dto.txnType}</u>
					</h3>
					<table cellpadding="0" cellspacing="0" align="center"
						style="font-size: 16px;">
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">TID</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">${dto.tid}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">MID</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.mid}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Date</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.date}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Time</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.time}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Card
								No</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.cardNo}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Holder</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.cardHolderName}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Card Type</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.cardType}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">RR
								Number</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.rrn}</td>
						</tr>
						
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Batch
								No</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.batchNo}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Appr
								Code</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.apprCode}</td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Trace
								No</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.traceNo}</td>
						</tr>
						
						
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-size: 16px; font-family: Lao UI;">Ref
								No</td>
							<td
								style="padding: 5px; text-align: right; font-size: 13px; font-family: Lao UI;">
								${dto.refNo}</td>
						</tr>

						<tr>
							<td
								style="padding: 10px; border-bottom: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
							<td
								style="padding: 10px; border-bottom: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
						</tr>

						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-family: Lao UI; font-size: 20px;">Amount</td>
							<td
								style="padding: 5px; text-align: right; font-family: Lao UI; font-size: 20px;">RM
								${dto.amount}</td>
						</tr>

						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-family: Lao UI; font-size: 20px;">Tip</td>
							<td
								style="padding: 5px; text-align: right; font-family: Lao UI; font-size: 20px;">RM
								${dto.additionAmount}</td>
						</tr>
						<tr>
							<td
								style="padding: 10px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
							<td
								style="padding: 10px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
						</tr>
						<tr>
							<td
								style="padding: 5px; color: #3F72D8; text-align: left; font-family: Lao UI; font-size: 20px;">Total</td>
							<td
								style="padding: 5px; text-align: right; font-family: Lao UI; font-size: 20px;">RM
								${dto.total}</td>
						</tr>

						<tr>
							<td
								style="padding: 1px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
							<td
								style="padding: 1px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;">
								<input type="hidden" name = "sign" id = "sign" value="${dto.sign}"></td>
						</tr>

						<tr id="tsign">
							<td></td>

							<td valign="top" style="padding-top: 5px;"  ><img width="150"
								src="data:image/jpg;base64,<c:out value='${dto.sign}'/>" /></td>
								
								


						</tr>
						<tr id="nsign">
							<td></td>

							
								
								<td valign="top" style="padding-top: 5px;"  ><small>Pin Verified</small></td>


						</tr>

						<tr>
							<td
								style="padding: 1px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
							<td
								style="padding: 1px; border-top: 1px dotted #ccc; color: #3F72D8; text-align: right;"></td>
						</tr>
					</table>
					 <h4 style="font-size: 13px;color: #0b9bff;display:block;text-align:center;">NO REFUND</h4>
					 
					   <!-- Map Location -->
		      	 <table cellpadding="0" cellspacing="0" align = "center" style="font-size:8px;">
				   <tr>
				  <td style="padding:2px ;text-align:center;font-family:Lao UI;font-size:8px;">
				   <!-- //+"<img width=\"250\" src=\"http://maps.googleapis.com/maps/api/staticmap?center=1.8668523,102.9440837&zoom=13&scale=false&size=400x300&maptype=roadmap&format=png&visual_refresh=true\" alt=\"Google Map of Mobiversa, KL\">"
				   //+"<img width=\"250\" src=\"http://maps.googleapis.com/maps/api/staticmap?center=1.8668523,102.9440837&size=400x400&type=map&zoom=13&imagetype=jpeg&showicon=red_1-1\" alt=\"Google Map of Mobiversa, KL\">"
				   //+"<a href=\"https://www.google.com/maps/place/1.8668523,102.9440837\"> <img width=\"250\" src=\"http://maps.google.com/maps/api/staticmap?center=1.8668523,102.9440837,&zoom=14&markers=1.8668523,102.9440837|1.8668523,102.9440837&path=color:0x0000FF80|weight:5|1.8668523,102.9440837&size=255x255&sensor=TRUE_OR_FALSE\" alt=\"Google Map of Mobiversa, KL\"> </a>" -->
				  <!-- <a href="https://www.google.com/maps/place/"+templetFields.getLatitude()+","+templetFields.getLongitude()+"\"> 
				  <img width=\"250\" src=\"http://maps.google.com/maps/api/staticmap?center="
				   +templetFields.getLatitude()+","+templetFields.getLongitude()+",&zoom=14&markers="+templetFields.getLatitude()+","+templetFields.getLongitude()+"|"
				   +templetFields.getLatitude()+","+templetFields.getLongitude()+"&path=color:0x0000FF80|weight:5|"
				   +templetFields.getLatitude()+","+templetFields.getLongitude()+"&size=255x255&sensor=TRUE_OR_FALSE\" alt=\"Google Map of Mobiversa, KL\"> </a>" -->
				   
				   <a href="https://www.google.com/maps/place/${dto.latitude},${dto.longitude}\"> 
				  <img width="250" src="http://maps.google.com/maps/api/staticmap?center=${dto.latitude},${dto.longitude},&zoom=14&markers=${dto.latitude},${dto.longitude}|${dto.latitude},${dto.longitude}&path=color:0x0000FF80|weight:5|${dto.latitude},${dto.longitude}&size=255x255&sensor=TRUE_OR_FALSE" alt="Google Map of Mobiversa, KL\"> </a>
				   </td>
				   </tr>
			       </table>
					 
			      <table cellpadding="0" cellspacing="0" align = "center" style="font-size:8px;">
				  <tr>
				   <td style="padding:2px ;text-align:center;font-family:Lao UI;font-size:8px;">
				   <small>I AGREE TO PAY ABOVE TOTAL AMOUNT ACCORDING TO CARD ISSUER AGREEMENT</small>
				   </td>
				   </tr>
				   <tr>
				   <td style="padding:2px ;text-align:center;font-family:Lao UI;font-size:9px;">
			       <small>MERCHANT COPY</small>
			       </td>
				   </tr>
			       </table>
				 
		</table>

	</table>

</body>
</html>
