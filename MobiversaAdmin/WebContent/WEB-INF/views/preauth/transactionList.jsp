...
<!-- <script type="text/javascript">
	...
	function loadSelectData() {
		...
		if (fromdateString == null || fromdateString == '' || todateString == null || todateString == '') {
			...
			document.location.href = '${pageContext.request.contextPath}/preauthtxn/search?date=' + fromdateString
					+ '&date1=' +todateString+'&txnType='+txnType + '&currPage=' + PageNumber;
			form.submit;
	...
	function loaddata() {
		...
		if (fromdateString == null || fromdateString == '' || todateString == null || todateString == '') {
			...
			//alert("test2: " + e + " " + e1);
			document.location.href = '${pageContext.request.contextPath}/preauthtxn/export?date=' + fromdateString
					+ '&date1=' + todateString+'&txnType='+ txnType + '&export=' + e2;
			//alert(e);
			form.submit();
	...
	function loadData(num){
		...
		e1=document.getElementById("date12").value;
		 if((e == null || e == '') && (e1 == null || e1 == '')){
			//alert('both $$ ##');
			document.location.href = '${pageContext.request.contextPath}/preauthtxn/list/'+pnum;
			form.submit;
		}else {
			//alert("else : "+e+" "+e1);
			document.location.href = '${pageContext.request.contextPath}/preauthtxn/search?date=' + e
					+ '&date1=' + e1+'&txnType=PREAUTH'+ '&currPage='+pnum;
      ...
      <div class="card blue-bg text-white">
        <div class="card-content">
          <div class="d-flex align-items-center">
           <h3 class="text-white">  <strong> EZYAUTH Summary</strong></h3>
          </div>
				...
				<div class="row">
					...
					<div class="input-field col s12 m3 l3">
						...
						<select name="txnType" id="txnType"
							...
							<option value="EZYAUTH">EZYAUTH</option>
			...
			<tbody>
				<c:forEach items="${paginationBean.itemList}" var="dto">
					<tr>
						...
						<td> <form method="get"
				action="${pageContext.request.contextPath}<%=PreAuthTxnController.URL_BASE%>/merchanttxn"> 
				
				       <input
						type="hidden"  name="date"  <c:out value="${dto.date}"/>>
					  <!-- <input
						type="hidden" name="date1" value="1130"> --> 
						<input
						type="hidden" name="merchantName" <c:out value="${dto.merchantName}"/>>
						<input
						type="hidden" name="txnType" <c:out value="${dto.txnType}"/>>
						<input
						type="hidden" name="city" <c:out value="${dto.location}"/>>
						 
						
				<button class="example_e1" type="submit" style="color:275ca8;"><font color="275ca8">${dto.merchantName}</font></button>
				<%-- <%-- <a
