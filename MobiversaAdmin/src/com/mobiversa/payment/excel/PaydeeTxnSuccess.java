package com.mobiversa.payment.excel;

import java.util.List;
import java.util.Map;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.apache.poi.hssf.usermodel.HSSFRow;
import org.apache.poi.hssf.usermodel.HSSFSheet;
import org.apache.poi.hssf.usermodel.HSSFWorkbook;
import org.springframework.web.servlet.view.document.AbstractExcelView;

import com.mobiversa.payment.util.forsettlement;


public class PaydeeTxnSuccess extends AbstractExcelView {
	
	@Override
	protected void buildExcelDocument(Map model, HSSFWorkbook workbook,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {

		HSSFSheet excelSheet = workbook.createSheet("Transaction List");
		setExcelHeader(excelSheet);
		
		List txnList = (List) model.get("TxnList");
		setExcelRows(excelSheet,txnList);
		
	}

	public void setExcelHeader(HSSFSheet excelSheet) {
		HSSFRow excelHeader = excelSheet.createRow(0);
		excelHeader.createCell(0).setCellValue("Date");
		excelHeader.createCell(1).setCellValue("Time");
		excelHeader.createCell(2).setCellValue("MID");
		excelHeader.createCell(3).setCellValue("TID");
		excelHeader.createCell(4).setCellValue("Amount");
		excelHeader.createCell(5).setCellValue("Name on Card");
		excelHeader.createCell(6).setCellValue("Card Number");
		excelHeader.createCell(7).setCellValue("Reference");
		excelHeader.createCell(8).setCellValue("AID Response");
		excelHeader.createCell(9).setCellValue("RRN");
		excelHeader.createCell(10).setCellValue("Status");
		excelHeader.createCell(11).setCellValue("Payment Method");
		excelHeader.createCell(12).setCellValue("MDR Amount");		
		excelHeader.createCell(13).setCellValue("Net Amount");
		excelHeader.createCell(14).setCellValue("Payment Date");
		
		
		

	}
	
	public void setExcelRows(HSSFSheet excelSheet, List<forsettlement> txnList){
		int record = 1;
		for (forsettlement forset : txnList) {
			HSSFRow excelRow = excelSheet.createRow(record++);
			excelRow.createCell(0).setCellValue(forset.getDate());
			excelRow.createCell(1).setCellValue(forset.getTime());
			excelRow.createCell(2).setCellValue(forset.getMid());
			excelRow.createCell(3).setCellValue(forset.getTid());
			excelRow.createCell(4).setCellValue(forset.getAmount());
			excelRow.createCell(5).setCellValue(forset.getNameoncard());
			excelRow.createCell(6).setCellValue(forset.getCardnumber());
			excelRow.createCell(7).setCellValue(forset.getReference());	
			excelRow.createCell(8).setCellValue(forset.getAidresponse());	
			excelRow.createCell(9).setCellValue(forset.getRrn());
			excelRow.createCell(10).setCellValue(forset.getStatus());
			excelRow.createCell(11).setCellValue(forset.getCardscheme()+ " " +forset.getCardtype());		
			excelRow.createCell(12).setCellValue(forset.getMdrAmount());
			excelRow.createCell(13).setCellValue(forset.getNetAmount());
			excelRow.createCell(14).setCellValue(forset.getSettlementDate());
			
			
		}
	}

}
