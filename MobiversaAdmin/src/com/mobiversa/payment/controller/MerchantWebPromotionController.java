...
@Controller
@RequestMapping(value = MerchantWebPromotionController.URL_BASE)
public class MerchantWebPromotionController extends BaseController {

	public static final String URL_BASE = "/promotionweb";
	...
	public String listMerchantPromotions(final Model model, @PathVariable final int currPage,
		...
		PageBean pageBean = new PageBean("MerchantWeb Promotion", "merchantweb/promotion/promotionList",
				Module.PROMOTION, "merchantweb/sideMenuMerchantWebMobile");
		...
		//logger.info("promiton serv");
		for (Promotion mp : paginationBean.getItemList()) 
		{
			
			//logger.info("created date: "+mp.getCreatedDate());
		}
	...
	public String displayAddMerchant(@ModelAttribute("merchtCustMail") MerchtCustMail merchtCustMail, Model model,
		 ...
		 logger.info("upload customer emaile file /custMailUpld");
		 HttpSession session=request.getSession();
			// logger.info("about to list all  transaction");
		String myName = (String) session.getAttribute("userName");
		//String myName = principal.getName();
		...
		PageBean pageBean = new PageBean("Merchant Customer Mail Upload",
				"merchantweb/promotion/custmail/custmailupload", Module.PROMOTION,
	...
	public String processAddUserForm(@ModelAttribute("merchtCustMail") final MerchtCustMail merchtCustMail,
			...
			final BindingResult errors, HttpSession session) {*/
	...
	// logger.info("about to Upload customer mail1");
		...
		PageBean pageBean = new PageBean("Merchant Customer Mail Upload",
				"merchantweb/promotion/custmail/custMailUploadReview", Module.PROMOTION,
				"merchantweb/sideMenuMerchantWebMobile");
		// start pay File
		...
		return "redirect:/promotionweb/merchtCustMailReviewAndConfirm";
	}
	...
	public String displayAddUserConfirmation(final ModelMap model, final HttpServletRequest request,
		...
		PageBean pageBean = new PageBean("Merchant user add Details", "merchantweb/promotion/custmail/custmailupldview",
				Module.PROMOTION, "merchantweb/sideMenuMerchantWebMobile");
		model.addAttribute("pageBean", pageBean);
		model.addAttribute("merchtCustMail", merchtCustMail);
		model.addAttribute("loginname", principal.getName());
	...
	public String confirmAddMerchant(@ModelAttribute("merchtCustMail") final MerchtCustMail merchtCustMail,
		...
		PageBean pageBean = new PageBean("Succefully New Merchant Added",
				"merchantweb/promotion/custmail/custmailupldsuccess", Module.PROMOTION, "agent/sideMenuAgent");
		...
		if (merchtCustMailSession == null) {
			return "redirect:/promotionweb/custMailUpld";
		}
		...
		logger.info("merchant details: "+currentMerchant.getUsername()+principal.getName());
	
		AuditTrail auditTrail=merchantService.updateAuditTrailByMerchant(currentMerchant.getUsername(), principal.getName(), "mailUpload");
			
		//AuditTrail auditTrail=promotionService.updateAuditTrailMerchantMailUpload(merchtCustMail.getMid());
		if(auditTrail.getUsername()!=null)
		{
			logger.info("Merchant :"+auditTrail.getUsername()+" Mail Uploaded Successfully by Merchant");
		}
		
		request.getSession(true).removeAttribute("merchtCustMailSession");
		model.addAttribute("merchant1", merchtCustMailSession);
	//	model.addAttribute("merchant",currentMerchant);
		model.addAttribute("pageBean", pageBean);
		model.addAttribute("loginname", principal.getName());
		return TEMPLATE_MERCHANT;
