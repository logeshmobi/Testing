...
//		return "admin/forgotPasswordDetailsNew";
//	}

//	new changes forgot pass
	...
	public String forgotPwdByUser(ModelMap model) {
		...
		model.addAttribute("heading", msg);
		try {
			List<String> validUsername = userDAO.getValidUsername();
			List<String> capitalizedUsernames = validUsername.stream()
					.filter(username -> username != null)   //filter null value (business names )in (merchant)(agent)(bank_user)
					.map(String::toUpperCase)
					.collect(Collectors.toList());
			ObjectMapper objectMapper = new ObjectMapper();
			String validUsernamesJson = "";
			try {
				validUsernamesJson = objectMapper.writeValueAsString(capitalizedUsernames);
			} catch (Exception e) {
				e.printStackTrace();
			}
			model.addAttribute("validUserNames", validUsernamesJson);
		} catch (Exception e) {
	...
	//forgot pass old code commented below
	
//	@RequestMapping(value = { "/forgetPwdByUserName" }, method = RequestMethod.POST)
//	public  String forgetPwdByUserName(
...
//
//
//				return "admin/resetPasswordDetails";
//
//			} else {
//				logger.info("Failure" );
...
//
//		}else {
//			statusMsg ="Please enter the registered valid User Name";
...
//	}
	...
	//forgot pass new flow and design start


	// new code karuppusamy & vignesh
	// for sending otp after clicking on send after
	@RequestMapping(value = {"/forgetPwdByUserName"}, method = RequestMethod.POST)
	public String forgetPwdByUserName(
			@RequestParam("username") String uname,
			@RequestParam(required = false, defaultValue = "1") final int currPage,
...
//                    model.addAttribute("isOtpValid", "true");
			...
			if (merchant == null || agent == null || bankUser == null) {
				statusMsg = "Invalid Username";
				model.addAttribute("statusMsg", statusMsg);
//            model.addAttribute("IsPassChanges", "true");
			}
...
//
			...
			if (merchant != null) {
				...
				checkOtpExpired(currentMerchantInfo.getOtpExpiryTime(), otp, otp1.getOptData(), model);
				model.addAttribute("merchantPass", merchant.getPassword());

			} else if (agent != null) {
				Agent currentAgent = agentService.loadAgent(agent.getUsername());
				checkOtpExpired(currentAgent.getOtpExpiryTime(), otp, otp1.getOptData(), model);
			} else if (bankUser != null) {
				BankUser currentBankuser = merchantService.loadBankUserByUsername(bankUser.getUsername());
				checkOtpExpired(bankUser.getOtpExpiryTime(), otp, otp1.getOptData(), model);
			} else {
...
//            return "admin/forgotPasswordDetailsSuccessNew";
	...
	public String checkOtpExpired(String OtpExpiryTime, String uiOtp, String dbOtp, Model model) {

		if (OtpExpiryTime == null) {
			logger.warn("OtpExpiryTime is null");
			return null;
		}
		...
		if (uiOtp.equalsIgnoreCase(dbOtp)) {
...
//	@RequestMapping(value = "/isOldPassMatches", method = RequestMethod.GET)
//	public ResponseEntity<String> isOldPassMatches(@RequestParam String username, @RequestParam String newPass) {
//		logger.info("Merchant Name: {} " + username);
//		logger.info("New Password: {}" + newPass);
//		List<String> merchantPassList = new ArrayList<>();
//		List<String> agentPassList = new ArrayList<>();
//		List<String> bankUserPassList = new ArrayList<>();
...
//			MerchantInfo merchantInfo = merchantService.loadMerchantInfoByFk(String.valueOf(merchant.getId()));
//			if (merchantInfo!=null) {
//				merchantPassList = Arrays.asList(
//						merchant.getPassword(),
//						merchantInfo.getPassword1(),
//						merchantInfo.getPassword2(),
//						merchantInfo.getPassword3()
//				);
//				logger.info("LIST :" + merchantPassList);
//				for (String pass : merchantPassList) {
...
//		if (agent != null) {
//
//			agentPassList = Arrays.asList(
//					agent.getPassword(),
//					agent.getPassword1(),
//					agent.getPassword2(),
//					agent.getPassword3()
//			);
//			logger.info("LIST :"+agentPassList);
//			for (String pass : agentPassList) {
//				logger.info("PASS "+pass);
//				if (encoder.matches(newPass, pass)) {
//					return new ResponseEntity<>("alreadyPresent", HttpStatus.OK);
//				}
...
//		}
//		if (bankUser != null) {
//			logger.info("COMING INTO BANK_USER :"+bankUser.getUsername());
//			bankUserPassList = Arrays.asList(
//					bankUser.getPassword(),
//					bankUser.getPassword1(),
//					bankUser.getPassword2(),
//					bankUser.getPassword3()
//			);
//			logger.info("LIST :"+bankUserPassList);
//			for (String pass : bankUserPassList) {
//				logger.info("PASS "+pass);
//				if (encoder.matches(newPass, pass)) {
//					return new ResponseEntity<>("alreadyPresent", HttpStatus.OK);
//				}
